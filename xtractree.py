...
class xtractTreeError(Exception):
    pass


class XtracTree:
    def __init__(
        self, estimator, x_train, x_test, sample_id=None, sample_ids=None, out=None
    ):
    ...
    def trees_to_dataframe(self):
        ...
        """
        if self.num_trees() == 0:
            msg = "There are no trees in this booster (LightGBM). Nothing to parse"
            raise xtractTreeError(msg)
    ...
    def build_dtree_rules(estimator, n_trees, x_train, l2w):
        ...
        for i in range(n_nodes):
            if is_leaves[i]:
                irow = bgn + "%s: return %s\n" % (
                    i,
                    ((value[i][0, 1] / weighted_n_node_samples[i]) / n_trees),
                )
            else:
    ...
    def build_model(self):
        ...
        if "LGBM" in str(self.estimator):
            ...
            if "RandomForestClassifier" in str(type(self.estimator)):
                ...
                for n in range(n_trees):
                    if n == 0:
                        l2w = XtracTree.build_dtree_rules(
                            self.estimator.estimators_[n], n_trees, self.x_train, l2w
                        )
                    else:
                ...
                n_trees = 1
                l2w = XtracTree.build_dtree_rules(
                    self.estimator, n_trees, self.x_train, l2w
                )
    ...
    def display_rule_per_estimator(estimator, X_test, sample_id, l2r, ndecisions):
    ...
    def sample_rules(self, ndecisions=None):
        ...
        if self.sample_id is not None:
            ...
            if "RandomForestClassifier" in str(type(self.estimator)):
                ...
                l2r = XtracTree.display_rule_per_estimator(
                    self.estimator, self.x_test, self.sample_id, l2r, ndecisions
                )
            # CONVERT l2r AS A DATAFRAME
            l2r = pd.DataFrame(
                l2r,
                columns=["Features", "Value Sample %s " % self.sample_id, "Threshold"],
            )
            ...
            if "RandomForestClassifier" in str(type(self.estimator)):
                ...
                for n in range(n_trees):
                    ...
                    XtracTree.display_rule_per_estimator_sample_ids(
                        self.estimator.estimators_[n], self.x_test, self.sample_ids
                    )
    ...
    def decisionsForForest(self, nDecisionsPerTree=5):
        ...
        tmpdf = XtracTree.sample_rules(
            self, ndecisions=nDecisionsPerTree
        )  # TOP 5 DECISIONS PER TREE
        resdf = XtracTree._decisions_forest(
            tmpdf, nDecisionsPerTree
        )  # AGGREGATE TOP DECISIONS
